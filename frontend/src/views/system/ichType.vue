...
const getTableData = async () => {
  const res = await http.get(
    `/ichType/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&name=${params.value.name}`
  );
  if (res.code === 200) {
...
const delRow = async (id: number) => {
  const res = await http.get(`/ichType/del?id=${id}`);
  if (res.code === 200) {
...
const batchDel = async () => {
  ...
  const res = await http.post("/ichType/batchDel", selectedIds.value);
  if (res.code === 200) {
...
const submit = async () => {
  ...
  if (isEdit.value) {
    ...
    const res = await http.post(`/ichType/add`, form.value);
    if (res.code === 200) {
